...
public class GameModel {
	...
	private void handleMapItemSelected(Room room) {
		if (getActualMap().isValidPath(getActualMap().getRoomPosX(getHeroPosition()),
				getActualMap().getRoomPosY(getHeroPosition()), getActualMap().getRoomPosX(room),
				getActualMap().getRoomPosY(room))) {
	...
	/**
	 * GameControllerのクリック検知を使用し、プレイヤーが実行しようとしたアクションを判断する
	 *
	 * @param x
	 * @param y
	 */
	...
	/**
	 * GameControllerのキーボード検知を使用し、プレイヤーが実行しようとしたアクションを判断する
	 *
	 * @param event
	 */
	...
	/**
	 * GameControllerのメインループ
	 * プレイヤーの位置に応じて、どのアクションを実行できるかを判断する
	 *
	 * @param x
	 * @param y
	 */
			...
			if (itemDrop) {
				...
				if (eventOnPosition == 4) {
					if (trader.getStore().containsKey(itemSelected)) {
						if (getTrader().getStore().get(itemSelected) <= backpack.getGold()
								.getAction()) {
							backpack.getGold()
									.changeQuantity(-getTrader().getStore().get(itemSelected));
	...
	private void handleFighting(int x, int y) {
		...
		if (heroTurn != 0) {
			...
			if (isSelected && confirmAttack != null) {
				if (confirmAttack.getClass() != Manastone.class
						&& confirmAttack.getClass() != Gold.class
						&& confirmAttack.getClass() != Arrow.class
						&& confirmAttack.getClass() != Magic.class) {
	...
	public void addScore(boolean isWin) {
		score.FinalScore(hero.getMaxHp(), hero.getLevel(), actualFloor, backpack.inventory(),
				isWin);
	}
