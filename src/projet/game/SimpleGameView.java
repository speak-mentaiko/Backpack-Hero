...
public record SimpleGameView(int xOrigin, int yOrigin, int height, int width, ImageLoader loader) {
	 ...
	 * @param color
	 */
	public static void drawStringR(ApplicationContext context, SimpleGameView view, String string, int i, int j,
	 ...
	 * @param dimY
	 */
	public static void draw(ApplicationContext context, SimpleGameView view, BufferedImage images, float x, float y,
	 ...
	 * @param alpha
	 */
	public static void drawRectangle(ApplicationContext context, SimpleGameView view, float x, float y, float width,
	 ...
	 * @param strokeWidth
	 */
	public static void drawRectangleOutline(ApplicationContext context, SimpleGameView view, float x, float y,
			float width, float height, Color color, float strokeWidth) {
		context.renderFrame(graphics -> {
			Graphics2D g2d = (Graphics2D) graphics;
			g2d.setColor(color);
	...
	public static void drawMap(ApplicationContext context, SimpleGameView view, RandomMap map) {
		...
		for (Room[] ligne : map.getCarte()) {
			...
			for (Room salle : ligne) {
				if (salle != null) {
					SimpleGameView.draw(context, view, view.loader.image("tilemap.png"),
							1920 / 2 - (11 * 75 / 2) + (75 * j), 50 + (75 * i), 75, 75);
					switch (salle.getApparition()) {
						case 1:
							SimpleGameView.draw(context, view, view.loader.image("IconeMob.png"),
									1920 / 2 - (11 * 75 / 2) + (75 * j), 50 + (75 * i), 75, 75);
							break;
						case 2:
							SimpleGameView.draw(context, view, view.loader.image("IconeChest.png"),
									1920 / 2 - (11 * 75 / 2) + (75 * j), 50 + (75 * i), 75, 75);
							break;
						case 3:
							SimpleGameView.draw(context, view, view.loader.image("Iconehealer.png"),
									1920 / 2 - (11 * 75 / 2) + (75 * j), 50 + (75 * i), 75, 75);
							break;
						case 4:
							SimpleGameView.draw(context, view, view.loader.image("IconeTrader.png"),
									1920 / 2 - (11 * 75 / 2) + (75 * j), 50 + (75 * i), 75, 75);
							break;
						default:
							break;
					...
					if (salle.getIsFinal()) {
						SimpleGameView.draw(context, view, view.loader.image("IconeStair.png"),
								1920 / 2 - (11 * 75 / 2) + (75 * j), 50 + (75 * i), 75, 75);
					}
				}
				j++;
	...
	public static void drawBackpack(ApplicationContext context, SimpleGameView view, Backpack backpack,
			GameModel data) {
		SimpleGameView.draw(context, view, view.loader.image("backpack.png"),
				(1920 / 2 - 180 - (backpack.backpack()[0].length * 90 / 2)), 10, 990, 550);
		for (int i = 0; i < backpack.backpack().length; i++) {
			for (int j = 0; j < backpack.backpack()[0].length; j++) {
				if (backpack.backpack()[i][j] != -1) {
					SimpleGameView.draw(context, view, view.loader.image("1x1_Shape.png"),
							(1920 / 2 - (backpack.backpack()[0].length * 90 / 2)) + (90 * j), 50 + (90 * i), 90, 90);
				} else if (data.isLevelUp() > 0 && !data.getIsFighting()) {
					SimpleGameView.draw(context, view, view.loader.image("1x1_Shape.png"),
							(1920 / 2 - (backpack.backpack()[0].length * 90 / 2)) + (90 * j), 50 + (90 * i), 90, 90);
					SimpleGameView.drawRectangle(context, view,
							(1920 / 2 - (backpack.backpack()[0].length * 90 / 2)) + (90 * j), 50 + (90 * i), 90, 90,
							Color.RED, 0.5f);
				}
			}
	...
	public static void drawItems(ApplicationContext context, SimpleGameView view, GameModel data) {
				...
				for (int j = 0; j < size[0].length; j++) {
					if (data.getIsOrganize()) {
						if (size[i][j] == 1 && (i != 0 || j != 0)) {
							drawRectangle(context, view, data.getItemSelected().getXY()[0] + j * 80,
									data.getItemSelected().getXY()[1] + i * 80, 80, 80, Color.YELLOW, 0.5f);
						} else if (i == 0 && j == 0) {
							drawRectangle(context, view, data.getItemSelected().getXY()[0],
									data.getItemSelected().getXY()[1], 80, 80, Color.BLUE, 0.5f);
						}
					} else if (data.getIsFighting() && data.getHeroTurn() != 0) {
						if (size[i][j] == 1) {
							drawRectangle(context, view, data.getItemSelected().getXY()[0] + j * 80,
									data.getItemSelected().getXY()[1] + i * 80, 80, 80, Color.RED, 0.5f);
						}
					}
				}
			}
		...
		for (Items i : data.getBackpack().inventory()) {
			SimpleGameView.draw(context, view, rotateImage(view.loader.image(i.getImgPath()), i.getRotation()),
					i.getXY()[0], i.getXY()[1], i.getXY()[3], i.getXY()[2]);
			if (i.getClass() == Manastone.class) {
				drawStringR(context, view, i.getAction() + "/" + ((Manastone) i).getMaximumMana(), i.getXY()[0] + 55,
						i.getXY()[1] + 60, 25, Color.WHITE);
			} else if (i.getClass() == Gold.class) {
				drawStringR(context, view, String.valueOf(i.getAction()), i.getXY()[0] + 55, i.getXY()[1] + 60, 25,
						Color.WHITE);
			}
		}
	}
	...
	public static void drawHero(ApplicationContext context, SimpleGameView view, GameModel data) {
		...
		if (data.getHero().getHp() * 100 / data.getHero().getMaxHp() >= 50) {
			color = Color.green;
		} else if (data.getHero().getHp() * 100 / data.getHero().getMaxHp() < 50
				&& data.getHero().getHp() * 100 / data.getHero().getMaxHp() >= 10) {
			color = Color.orange;
		} else {
			color = Color.red;
		}
		SimpleGameView.drawRectangle(context, view, 380, 725, data.getHero().getHp() * 100 / data.getHero().getMaxHp(),
				20, color, 1f);
	...
	public static void drawBeasts(ApplicationContext context, SimpleGameView view, GameModel data) {
		...
		for (Bestiary beast : data.getEnemyInFight()) {
			...
			SimpleGameView.drawRectangle(context, view, 1280 + i, 725, beast.getHealth() * 100 / beast.getMaxHealth(),
					20, color, 1f);
	...
	public static void drawCard(ApplicationContext context, SimpleGameView view, float x, float y, float width,
			float height, int font, String string) {
			...
			if (item.getCost() != null) {
				for (var cost : item.getCost().keySet()) {
					if (item.getCost().get(cost) != 0) {
						...
						drawStringR(context, view, String.valueOf(item.getCost().get(cost)), 1622 + i, 60, 30,
								Color.WHITE);
					}
					i += 60;
				}
	...
	public static void drawInterface(ApplicationContext context, SimpleGameView view, GameModel data) {
		...
		drawCard(context, view, 230, -20, 100, 200, 20, "Lvl " + data.getHero().getLevel() + " : "
				+ data.getHero().getXP() + "/" + data.getHero().getPalierXP(data.getHero().getLevel()));
	...
	public static void drawEffects(ApplicationContext context, SimpleGameView view, GameModel data,
			HashMap<String, Integer> effets, int x, int y) {
